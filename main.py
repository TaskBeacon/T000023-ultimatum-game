...
def run(options: TaskRunOptions):
    """Run Ultimatum Game in human/qa/sim mode with one auditable flow."""
    ...
    with runtime_scope:
        ...
        for block_i in range(settings.total_blocks):
            ...
            accept_rate = (
                sum(1 for trial in block_trials if bool(trial.get("accepted", False))) / len(block_trials)
                if block_trials
                else 0.0
            )
...
def main() -> None:
    ...
    options = parse_task_run_options(
        task_root=task_root,
        description="Run Ultimatum Game in human/qa/sim mode.",
